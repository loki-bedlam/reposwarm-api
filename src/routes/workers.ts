...
router.post('/workers/:id/inference-check', async (req: Request, res: Response) => {
  // Use shared inference client — all provider/auth logic in one place
  const result = await infer({
    prompt: 'Say OK',
    maxTokens: 10,
  })

  logger.info({ provider: result.provider, model: result.model, authMethod: result.authMethod, latencyMs: result.latencyMs }, 
    result.success ? 'Inference check passed' : 'Inference check failed')
